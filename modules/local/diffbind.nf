process DIFFBIND {
    ...
    tryCatch({
        ...
        db_obj <- dba.normalize(db_obj, method=DBA_ALL_METHODS, normalize=DBA_NORM_NATIVE, background=TRUE)
        ...
        if (!is.null(analysis_status) && !is.null(analysis_status\$contrasts)) {
            ...
            if(!is.null(res_db)) {
                ...
                if(length(res_db) > 0) {
                    ...
                    # Salviamo il LogFC (Fold) per permettere lo split Gain/Loss in Profileplyr!
        ...
        cat(paste0(
            ...
            "  <h3>DiffBind Analysis: ", "${prefix}", "</h3>\\n",
            "  <p><strong>Avviso:</strong> L'analisi differenziale non è stata completata (es. campioni insufficienti o dati anomali).</p>\\n",
            "  <p><em>Dettaglio tecnico: ", e\$message, "</em></p>\\n",
            "</div>"
        ), file="${prefix}_diffbind_error_mqc.html")
        
        # Nota: il file _sig_peaks.bed è già stato creato vuoto all'inizio, 
        # quindi Nextflow troverà il file, non darà errore e Profileplyr userà il fallback!
    })
